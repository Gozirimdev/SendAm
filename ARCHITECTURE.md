...
## Wallets: wallet provisioning, not wallet mapping

SendAm does not generate or hold private keys itself. `apps/api/src/wallet/`
is a thin abstraction (`WalletService`) over a managed wallet provisioning
provider — Provider Engine by default, with Vendor scaffolded as a
...
pattern (a `chains/` module with a `chain.reader.js` / `lisk.reader.js`
interface, each user holding both a chain and a Lisk wallet). That
approach is not part of this codebase — the wallet provisioning model was
...
branch (tip commit `d770f2c`) if wallet mapping is ever revisited.
...
| Managed wallets (Provider/Vendor) | Full client | Custody, key management |
