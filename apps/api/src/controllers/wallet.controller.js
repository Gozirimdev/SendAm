const { createWalletForUser, getWalletByPhoneNumber, getWalletByUserId, markWalletFunded } = require('../services/account.service');
const { getBalance, fundAccount, isValidPublicKey } = require('../services/chain.service');
...
const sendFunds = async (req, res, next) => {
  try {
    ...
    if (!isValidPublicKey(destination)) {
      return sendError(res, 'Destination must be a valid chain public key');
    }
    ...
    const result = await executeSend({ user, wallet, destination, amount, asset: 'TOKEN' });
    if (!result.ok) {
