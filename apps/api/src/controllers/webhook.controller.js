...
const { getBalance, getTransactionUrl, isValidPublicKey, sendToken, fundAccount } = require('../services/chain.service');
...
const processCommand = async (phoneNumber, whatsappName, text) => {
  ...
  switch (command.type) {
    ...
    case 'INVALID_SEND':
      await sendTextMessage(phoneNumber, `Invalid send format. Please use: send <amount> token <address-or-name>\nExample: send 5 token GABC...`);
      break;

    case 'INVALID_SAVE':
      await sendTextMessage(phoneNumber, `Invalid save format. Please use: save <name> <chain-address>\nExample: save ada GABC...`);
      break;
...
const handleSaveContact = async (phoneNumber, user, payload) => {
  try {
    ...
    if (!isValidPublicKey(publicKey)) {
      await sendTextMessage(phoneNumber, `That is not a valid chain public key. Please check the address and try again.`);
      return;
    }
...
const handlePrepareSend = async (phoneNumber, user, payload) => {
  try {
    ...
    if (!resolvedRecipient.destination) {
      await sendTextMessage(phoneNumber, `I could not find "${recipient}" in your contacts, and it is not a valid chain public key.\n\nUse: save ${recipient.toLowerCase()} GABC...`);
      return;
    }
...
const handleConfirmSend = async (phoneNumber, user) => {
  try {
    if (!user.pendingSend?.destination || isPendingSendExpired(user.pendingSend)) {
      ...
      await user.save();
      await sendTextMessage(phoneNumber, `No active transfer to confirm. Send a new command like: send 5 token GABC...`);
      return;
    }
    ...
    await sendTextMessage(phoneNumber, `Processing your transfer of ${amount} TOKEN...`);
