// Central startup check. secretStore.js and adminAuth.service.js already
...
// found — an operator fixing SERVICE_SECRET only to hit JWT_SECRET on the
...
const validateEnv = (config) => {
  ...
  if (!config.encryptionKey || Buffer.from(config.encryptionKey, 'hex').length !== 32) {
    problems.push('SERVICE_SECRET must be a 64-character hex string (32 bytes). Generate one with: openssl rand -hex 32');
  }
