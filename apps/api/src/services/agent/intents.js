...
const parseIntent = (text) => {
  ...
  if (normalizedText.startsWith('save ')) {
    ...
    // Casing is left as typed — chain keys get normalized to uppercase
    // downstream, but forcing case here would corrupt an EVM address (Lisk
    // addresses aren't simply upper/lowercase the way AddressCodec is).
    if (parts.length === 3) {
      return {
        type: 'SAVE_CONTACT',
        payload: {
  ...
  if (normalizedText.startsWith('send ')) {
    const parts = trimmedText.split(/\s+/);
    // format: send <amount> <asset> <address-or-name>, e.g. "send 5 token ada"
    ...
    // from this keyword, so 'token'/'eth' here is just user-facing labeling.
    if (parts.length >= 4) {
      return {
        type: 'SEND',
        payload: {
