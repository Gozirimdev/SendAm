...
// Attempt to (re)fund one wallet and return its resulting status. chain
// auto-funds via faucet (retried inside the adapter). Lisk has no
// equivalent auto-fund API — see lisk.reader.js — so instead we check
...
const handleSaveContact = async ({ phoneNumber, user, payload }) => {
  try {
    ...
    // chain keys are conventionally stored uppercase (AddressCodec); EVM
    // addresses are left as typed since case carries checksum meaning.
    const normalizedPublicKey = chain === 'chain' ? publicKey.toUpperCase() : publicKey;
...
const resolveRecipient = (user, recipient) => {
  ...
  if (chain) {
    const destination = chain === 'chain' ? normalizedRecipient.toUpperCase() : normalizedRecipient;
  ...
  return { destination: contact.publicKey, alias: contact.alias, chain: contact.chain || 'chain' };
};
