...
const chainLabel = (chain) => (chain === 'lisk' ? 'Lisk' : 'chain');
...
const replies = {
  ...
  help: () =>
    [
      'Available commands:',
      '- create wallet: Create your chain and Lisk wallets',
      '- fund: Retry funding any wallet that is not yet funded',
      '- balance: Check your balances across chains',
      '- save <name> <address>: Save a contact (chain or Lisk address)',
  ...
  // Wallet
  creatingWallet: () => `Creating your chain and Lisk wallets...`,
  ...
  // Contacts
  invalidAddress: () =>
    `That is not a valid chain or Lisk address. Please check it and try again.`,
  contactSaved: (alias, publicKey, chain) =>
    `Saved ${alias} (${chainLabel(chain)}) as ${shortenPublicKey(publicKey)}.\n\nYou can now send with: send 5 ${chain === 'lisk' ? 'eth' : 'token'} ${alias}`,
