...
const { sendPayment, getTransactionUrl } = require('./chain.service');
...
const enforceSendLimits = async (userId, amount) => {
  ...
  if (parsed > limits.maxSendAmount) {
    throw new Error(`Amount exceeds the per-transaction limit of ${limits.maxSendAmount} TOKEN.`);
  }
  ...
  if (sentToday + parsed > limits.dailySendAmount) {
    ...
    throw new Error(`This transfer exceeds your daily limit of ${limits.dailySendAmount} TOKEN (${remaining.toFixed(2)} TOKEN remaining today).`);
  }
};
 ...
 */
const executeSend = async ({ user, wallet, destination, amount, asset = 'TOKEN' }) => {
  try {
