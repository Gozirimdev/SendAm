// Lisk (EVM L2) implementation of the chain-adapter interface (see ./index.js).
...
// Native asset only for now. Same seam as the chain adapter's resolveAsset —
...
// Honest limitation: Lisk Sepolia has no faucet-equivalent auto-fund API.
// Testnet funding goes through human/device-attestation faucets (Superchain
// Faucet, L2 Faucet), which can't be driven server-side the way faucet
// can. Callers must branch on `manual` and surface `instructions` instead of
// assuming the wallet gets funded the way the chain side does.
