// chain leg of a Lisk -> chain USDC bridge, via the real Allbridge Core
...
//      Lisk's native OP-Stack canonical bridge -> Ethereum -> chain via
...
//      paths — see initiateBridgeTochainUsdc below.
//
//   3. Allbridge Core's "chain" USDC actually lives on the Soroban
//      (smart-contract) side, not classic chain — verified live:
...
//      chain integration (chain.reader.js) is classic-chain
//      (rpc accounts, ChainSdk.Asset/trustlines) — it does not talk
...
const getSdk = () => {
  if (!sdk) {
    ...
    // SDK's own docs, and this adapter only touches the chain (STLR) leg.
 ...
 * Allbridge-supported chain into chain USDC. Safe to call in any
 * environment — fetches live pricing/route data from Allbridge's Core API
 * and moves nothing.
 */
const quoteTochainUsdc = async ({ fromChainSymbol, amount, messenger }) => {
 ...
 * the chain-side Provider (transaction signer) still needs to be
 ...
 */
const initiateBridgeTochainUsdc = async () => {
  throw new Error(
    'Not implemented: Allbridge Core has no testnet, so executing this requires ' +
    'a real, deliberately-constructed chain Provider and real mainnet funds. ' +
