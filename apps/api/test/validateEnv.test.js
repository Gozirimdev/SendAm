...
test('missing SERVICE_SECRET throws', () => {
  const config = baseConfig();
  config.encryptionKey = undefined;
  assert.throws(() => validateEnv(config), /SERVICE_SECRET/);
});

test('short SERVICE_SECRET throws', () => {
  const config = baseConfig();
...
test('multiple violations are all reported in one error', () => {
  ...
  assert.throws(() => validateEnv(config), (err) => {
    assert.match(err.message, /SERVICE_SECRET/);
