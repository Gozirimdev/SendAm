...
//
// What this proves: the chain leg of "bridge value into chain USDC" is
...
//   3. The USDC this bridges into lives on Soroban, not classic chain —
...
//      the way native TOKEN is today.
const mongoose = require('mongoose');
const config = require('../src/config/env');
const { ChainSymbol, quoteTochainUsdc } = require('../src/services/chains/bridge.adapter');
...
const run = async () => {
  ...
  console.log(`Quoting the real, supported route instead: Ethereum USDC -> chain USDC (amount: ${SPIKE_AMOUNT})\n`);

  const { sourceToken, destinationToken, amountToBeReceived } = await quoteTochainUsdc({
  ...
  console.log(`  Destination: ${destinationToken.symbol} on ${destinationToken.chainSymbol} (${destinationToken.tokenAddress}) — Soroban, not classic chain`);
  ...
  const entry = await BridgeLedgerEntry.create({
    ...
    status: 'observed',
    notes: 'Groundwork spike only. Quoted via Ethereum (the real, supported chain) as a stand-in for Lisk, which Allbridge Core does not support. Destination is Soroban USDC, not classic chain. No funds moved. Not wired into the live bot.',
  });
