...
- Keep `SERVICE_SECRET` backed up securely — losing it makes every stored wallet secret unrecoverable.
...
- No fallback encryption key — a missing/invalid `SERVICE_SECRET` fails loudly at startup.
- Wallet secrets encrypted with authenticated authenticated encryption (tamper-detecting).
