...
WhatsApp-first payments with wallet-mapping wallets, voice-to-cash, escrow, nearby cash-out, and automatic payment-rail routing.

SendAm maps a WhatsApp phone number to a chain wallet and a Lisk wallet and lets users send, receive, escrow, check balances, request receipts, and find cash-out options from chat. The user experience hides blockchain complexity: the backend decides whether a payment should use Lisk, chain corridor rails, or a fiat on/off-ramp provider, based on the destination address.
...
- Phone number as wallet identity — every user gets a chain and a Lisk wallet.
- Wallet mapping: keys generated and encrypted (authenticated encryption) locally, not managed by a third-party provider.
...
- `wallet`: Direct-custody chain adapters (chain, Lisk) plus a `WalletService` abstraction for create/get wallet, send, balance, and transaction history. App code never imports a chain SDK directly — see `chainIndex.js`.
...
- Support non-native assets per chain (chain `changeTrust`, Lisk bridged/ERC-20 USDC).
...
- Wallet private keys encrypted with authenticated authenticated encryption (tamper-detecting); the server fails fast at startup without a valid `SERVICE_SECRET`.
...
- Add secure, managed secret/key management (KMS/HSM) instead of a single static `SERVICE_SECRET` for wallet private keys; support key rotation.
